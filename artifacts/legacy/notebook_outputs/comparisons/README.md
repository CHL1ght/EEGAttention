...
这里保存早期方法之间的比较表，不是当前冻结模型的独立测试成绩。
...
|---|---|---|---|
| [comparison_results_original_vs_self.csv](comparison_results_original_vs_self.csv) | 上游/自采流程的历史比较表；仅作探索性记录。 | 采集或程序生成 | 不就地覆盖；需另存版本并留痕 |
| [edf_mne_channel_info.xlsx](edf_mne_channel_info.xlsx) | 用 MNE 检查 EDF 通道名称、类型和数量的表格。 | 采集或程序生成 | 不就地覆盖；需另存版本并留痕 |
