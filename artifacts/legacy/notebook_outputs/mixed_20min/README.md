...
这里保存早期约20分钟混合状态录制在 notebook 中处理后导出的中间结果。
...
|---|---|---|---|
| [data_0001_raw_export.xlsx](data_0001_raw_export.xlsx) | 旧 notebook 对过短 demo EDF 的导出/检查表；该 EDF 短于 4 秒，不进入正式训练。 | 采集或程序生成 | 不就地覆盖；需另存版本并留痕 |
