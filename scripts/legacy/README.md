...
这里保留旧上游深度学习和ROC曲线的辅助脚本，不是当前三模型推理入口。
...
|---|---|---|---|
| [eeg_deep_worker.py](eeg_deep_worker.py) | 单个上游深度模型/特征变体的 worker。 | 手写维护 | 可维护，保留来源与实验边界 |
| [README.md](README.md) | 本目录为什么存在、属于哪一步，以及各文件怎么看。 | 手写维护 | 可维护，保留来源与实验边界 |
| [run_deep_workers.py](run_deep_workers.py) | 批量调度上游深度 worker。 | 手写维护 | 可维护，保留来源与实验边界 |
| [save_roc_figures.py](save_roc_figures.py) | 读取上游 ROC 缓存并保存图片。 | 手写维护 | 可维护，保留来源与实验边界 |
