...
REPRODUCTION_DIR = PROJECT_DIR / "artifacts" / "reproductions" / "upstream_pipeline"
...
def output_paths(model_name, variant):
    ...
    return {
        "model": REPRODUCTION_DIR / "models" / f"{model_name}_model_part_detrend_plr_stft_bin_win{suffix}.pth",
        "xlsx": REPRODUCTION_DIR / "results" / f"train_info_{base}.xlsx",
        "roc": REPRODUCTION_DIR / "results" / "roc_artifacts" / f"{base}.npz",
    }
...
def main():
    ...
    parser.add_argument("--cache-dir", default=str(REPRODUCTION_DIR / "feature_cache"))
    ...
    (REPRODUCTION_DIR / "models").mkdir(parents=True, exist_ok=True)
    (REPRODUCTION_DIR / "results" / "roc_artifacts").mkdir(parents=True, exist_ok=True)
    progress_dir = REPRODUCTION_DIR / "results" / "progress"
