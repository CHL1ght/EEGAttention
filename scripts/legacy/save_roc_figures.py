...
REPRODUCTION_RESULTS_DIR = PROJECT_DIR / "artifacts" / "reproductions" / "upstream_pipeline" / "results"
...
def load_roc(model_key, suffix):
    path = REPRODUCTION_RESULTS_DIR / "roc_artifacts" / artifact_name(model_key, suffix)
...
def save_variant_figure(variant, meta):
    ...
    out_path = REPRODUCTION_RESULTS_DIR / meta["file"]
...
def main():
    REPRODUCTION_RESULTS_DIR.mkdir(parents=True, exist_ok=True)
